...
export async function POST(request:Request){
  ...
  try {const response=await fetch(argoApi('/query'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({query:body.query,mode:body.mode||'exact'}),signal:AbortSignal.timeout(35000)});
